...
**Updated:** 2026-08-28, from direct photographic evidence (serial/compliance labels, an Anritsu boot screen) supplied by Radianode.
...
| Last calibration date | **Not established.** PPK2 units are commonly factory-calibrated only, with no field recalibration path — but this has not been confirmed for this specific unit. Check Nordic's documentation or the unit's own records if any exist. |
...
| Last calibration date | 2025. Precise date **not established** — only the year is currently known. |
...
3. Check for a calibration sticker (often shows last-cal and next-due dates) or pull the instrument's calibration certificate/record if Radianode maintains one. (Remaining: PPK2; HP 6611C's precise date beyond "2025"; Anritsu; DMM; Keysight scope.)
